defmodule GoogleSheets.Loader.FileSystem do
  ...
  defp load_spreadsheet(previous_version, dir, sheets) do
    ...
    if not Enum.all?(sheets, fn sheetname ->
         Enum.any?(worksheets, fn ws -> ws.name == sheetname end)
       end) do
      loaded =
        worksheets
      ...
      throw(
        {:error,
         "All requested worksheets were not found, expected to load #{inspect(sheets)} loaded: #{
           inspect(loaded)
         }"}
      )
    end
  ...
  defp calculate_version(worksheets) when is_list(worksheets) do
    concatenated =
      ...
      |> Enum.reduce("", fn ws, acc -> ws.csv <> acc end)
